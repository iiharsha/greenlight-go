...
func (m MovieModel) Update(movie *Movie) error {
	// update SQL query and returning the new version number
	query := `
	UPDATE movies
...
func (m MovieModel) Delete(id int64) error {
	...
	//execute the SQL query using the Exec() method, passing in the id
...
func ValidateMovie(v *validator.Validator, movie *Movie) {
	...
	v.Check(movie.Runtime > 0, "runtime", "must be postive integer")
...
func (m MovieModel) GetAll(title string, genres []string, filters Filters) ([]*Movie, Metadata, error) {
	...
	//collect values of limit() and offset() helpers into a slice
